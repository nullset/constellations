...
const allStore = new WeakMap();
window.allStore = allStore;
...
// Hidden variables
export const isBlissElement = Symbol("isBlissElement");
export const componentHasLoaded = Symbol("componentHasLoaded");
...
// Function that defines the custom element.
function define(tagName, mixins = [], options = {}) {
  ...
  // Build the actual web component class here. It is based on a chain of different classes.
  class BlissElement extends baseClass {
    [isBlissElement] = true;

    // Gives a list of all the observed props/attributes.
    ...
    // Get the store for this element from the global store.
    get $() {
      return allStore.get(this);
    }

    constructor() {
      super();

      // Create element's observable state.
      const $ = observable(Object.create(null));
      // Save it on the global store WeakMap.
      allStore.set(this, $);
      ...
      Object.entries(flattenedPrototype.props).forEach(([prop, value]) => {
        ...
        observe(() => {
          let convertedValue =
            this.$[prop] == null
              ? null
              : this.typecastValue(converter, this.$[prop]);
  ...
  // This means you can do something like the following to set a value:
  //        `myElem.myObservedAttr = 4`;
  // and it will, behind the scenes, call:
  //        `myElem.$.myObservedAttr = 4`
  // and similarly calling the getter
  //        `myElem.myObservedAttr`
  // will return
  //        `myElem.$.myObservedAttr`.
